                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isCorrect && "bg-green-500/10 text-green-700 dark:text-green-400",
                          isSelected && !isCorrect && "bg-red-500/10 text-red-700 dark:text-red-400"
                        )}
                      >
                        {isCorrect ? <CheckCircle className="h-4 w-4" /> : (isSelected ? <XCircle className="h-4 w-4" /> : <div className="w-4 h-4"/>)}
                        <span>{answer}</span>
                      </div>
                    );
                  })}
          ...
          >
            {q.answers.map((answer, aIndex) => (
              <div key={aIndex} className="flex items-center space-x-2">
                ...
                <Label htmlFor={`q${qIndex}a${aIndex}`} className="font-normal cursor-pointer">
                  {answer}
                </Label>
              </div>
            ))}
